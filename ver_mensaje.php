...
$varUsuario_consulta_mensaje = 0;
if (isset($_GET['recordID'])) {
  $varUsuario_consulta_mensaje = $_GET['recordID'];
}
$varUS_consulta_ninera = "0";
if (isset($_SESSION['MM_id_numn'])) {
  $varUS_consulta_ninera = $_SESSION['MM_id_numn'];
}
//CONSULTA SQL LOS MENSAJES DEL USUARIO
mysql_select_db($database_Conexionnany, $Conexionnany);
$query_consulta_mensajes = sprintf("SELECT * FROM mensajes WHERE mensajes.id_ninera = %s AND mensajes.id_mensaje = %s", 
                    GetSQLValueString($varUS_consulta_ninera, "int"),               
                    GetSQLValueString($varUsuario_consulta_mensaje, "int"));
...
<html>
	<head>
		<title>Mensaje de <?php echo $row_consulta_mensajes['mensaje_de']; ?> </title>
		<meta charset="utf-8" />
...
<p>Mensaje de: <?php echo $row_consulta_mensajes['mensaje_de']; ?></p>
<p>Fecha: <?php echo $row_consulta_mensajes['fecha_mensaje']; ?>
<p><?php echo $row_consulta_mensajes['mensaje_txt']; ?></p>
</br>
<a href="mensaje_leido.php"><input type="submit" style="width:300px;height:50px" value="Marcar como terminado" /></a>
